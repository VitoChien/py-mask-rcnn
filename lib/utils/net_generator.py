...
def conv_factory(name, bottom, ks, nout, stride=1, pad=0):
    ...
    if "res" in name:
        ...
        batch_norm = L.BatchNorm(conv, name = "bn_" + name_new, in_place=True, batch_norm_param=dict(use_global_stats=False))
        scale = L.Scale(batch_norm, name = "scale_" + name_new, in_place=True, scale_param=dict(bias_term=True))
    else:
        batch_norm = L.BatchNorm(conv, name = "bn_" + name, in_place=True, batch_norm_param=dict(use_global_stats=False))
...
def conv_factory_inverse_no_relu(name, bottom, ks, nout, stride=1, pad=0):
    conv = L.Convolution(bottom, name = name, kernel_size=ks, stride=stride,
...
def residual_block_shortcut(name, bottom, num_filter, stride=1):
    conv1 = conv_factory(name + "_branch2a", bottom, 1, num_filter, stride, 0)
    conv2 = conv_factory(name + "_branch2b", conv1, 3, num_filter, stride, 1)
    conv3 = conv_factory_inverse_no_relu(name + "_branch2c", conv2, 1, 4 * num_filter, stride, 0)
    addition = L.Eltwise(bottom, conv3, name = name, operation=P.Eltwise.SUM)
    relu = L.ReLU(addition, name = name + "_relu" , in_place=True)
    return relu


def residual_block(name, bottom, num_filter, stride=1):
    conv1 = conv_factory(name + "_branch2a",bottom, 1, num_filter, stride, 0)
    conv2 = conv_factory(name + "_branch2b",conv1, 3, num_filter, 1, 1)
    conv3 = conv_factory_inverse_no_relu(name + "_branch2c", conv2, 1, 4 * num_filter, 1, 0)
...
def residual_block_shortcut_basic(name, bottom, num_filter, stride=1):
    conv1 = conv_factory(name + "_branch2b", bottom, 3, num_filter, stride, 1)
    conv2 = conv_factory_inverse_no_relu(name + "_branch2c", conv1, 3, 4 * num_filter, stride, 1)
    addition = L.Eltwise(bottom, conv2, name = name, operation=P.Eltwise.SUM)
    relu = L.ReLU(addition, name = name + "_relu" , in_place=True)
    return relu


def residual_block_basic(name, bottom, num_filter, stride=1):
    conv1 = conv_factory(name + "_branch2b",bottom, 3, num_filter, 1, 1)
    conv2 = conv_factory_inverse_no_relu(name + "_branch2c", conv1, 3, 4 * num_filter, 1, 0)
    conv1_2 = conv_factory_inverse_no_relu(name + "_branch1", bottom, 1, 4 * num_filter, stride, 0)
...
def rpn(bottom, gt_boxes, im_info, data, anchors = 9, train = True):
...
def roi_proposals(rpn_cls_score_reshape, rpn_bbox_pred, im_info, gt_boxes, classes, feat_stride = 16):
        ...
        for j in range(i):
            if j==0:
                ...
                if module == "normal":
                    ...
                    out = residual_block_basic("res" + str(index) + ascii_lowercase[j], out, channals, stride)
            else:
                if module == "normal":
                    out = residual_block_shortcut("res" + str(index) + ascii_lowercase[j], out, channals)
                else:
                    out = residual_block_shortcut_basic("res" + str(index) + ascii_lowercase[j], out, channals)
        channals *= 2
    return to_proto(out)

def resnet_rcnn(stages=[3, 4, 6, 3], channals=64, deploy=False, classes = 2, anchors = 9, feat_stride = 16, pooling = "align", module = "normal", train=True):
    ...
    for i in stages[:-1]:
        ...
        channals *= 2
    if train:
        rpn_cls_loss, rpn_loss_bbox, rpn_cls_score_reshape, rpn_bbox_pred = rpn(out, gt_boxes, im_info, data, train)
    else:
        rpn_cls_score_reshape, rpn_bbox_pred = rpn(out, gt_boxes, im_info, data, train)
    rois, labels, bbox_targets, bbox_inside_weights, bbox_outside_weights = \
        roi_proposals(rpn_cls_score_reshape, rpn_bbox_pred, im_info, gt_boxes, classes)
